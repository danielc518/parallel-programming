...
void serialFilterFirst ( int data_len, unsigned int* input_array, unsigned int* output_array, int filter_len, unsigned int* filter_list, FILE *result )
{
...
/* Function to apply the filter with the filter list in the outside loop */
void serialDataFirst ( int data_len, unsigned int* input_array, unsigned int* output_array, int filter_len, unsigned int* filter_list, FILE *result )
{
	/* Variables for timing */
	struct timeval ta, tb, tresult;
...
/* Function to apply the filter with the filter list in the outside loop */
void parallelFilterFirst ( int data_len, unsigned int* input_array, unsigned int* output_array, int filter_len, unsigned int* filter_list, FILE *result )
{
	/* Variables for timing */
	struct timeval ta, tb, tresult;
...
/* Function to apply the filter with the filter list in the outside loop */
void parallelDataFirst ( int data_len, unsigned int* input_array, unsigned int* output_array, int filter_len, unsigned int* filter_list, FILE *result )
{
	/* Variables for timing */
	struct timeval ta, tb, tresult;
...
int main( int argc, char** argv )
{
	...
	FILE *ff = fopen("pff_speed.csv", "w");
	FILE *df = fopen("pdf_speed.csv", "w");
	...
	for ( int num_runs=1; num_runs<=max_runs; num_runs++)
	{
		...
		/* Execute at a variety of # threads */
		for ( int num_threads=1; num_threads<=max_threads; num_threads*=2)
		{
			omp_set_num_threads(num_threads);

			parallelDataFirst ( DATA_LEN, input_array, serial_array, filter_len, filter_list, df );
			memset ( output_array, 0, DATA_LEN );

			parallelFilterFirst ( DATA_LEN, input_array, output_array, filter_len, filter_list, ff );
	...
	ff = fopen("pff_scale.csv", "w");
	df = fopen("pdf_scale.csv", "w");
