...
int main( int argc, char** argv )
{
	...
	FILE *ff = fopen("ff.csv", "w");
	FILE *df = fopen("df.csv", "w");
	...
	for ( int num_runs=1; num_runs<=max_runs; num_runs++)
	{
		...
		{
			serialDataFirst ( DATA_LEN, input_array, serial_array, filter_len, filter_list, df );
			memset ( output_array, 0, DATA_LEN );

			serialFilterFirst ( DATA_LEN, input_array, output_array, filter_len, filter_list, ff );
